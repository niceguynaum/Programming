...
Copyright (c) 1999 - 2018 by PJ Naughter. (Web: www.naughter.com, Email: pjna@naughter.com)
...
#ifdef CSERIALPORT_MFC_EXTENSIONS
  BOOL GetErrorMessage(_Out_writes_z_(nMaxError) LPTSTR lpszError, _In_ UINT nMaxError, _Out_opt_ PUINT pnHelpContext = nullptr) override
  {
    return GetErrorMessage2(lpszError, nMaxError, pnHelpContext);
  }
...
__if_exists(OpenCommPort)
{
  ...
  BOOL Open(_In_z_ LPCTSTR pszPort, _In_ DWORD dwBaud = 9600, _In_ Parity parity = NoParity, _In_ BYTE DataBits = 8,
            _In_ StopBits stopBits = OneStopBit, _In_ FlowControl fc = NoFlowControl, _In_ BOOL bOverlapped = FALSE) noexcept
  {
...
#ifdef CSERIALPORT_MFC_EXTENSIONS
#pragma warning(suppress: 26409 26400 26462)
